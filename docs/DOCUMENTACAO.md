...
- **Biblioteca de currículos** — gerenciamento centralizado dos currículos enviados pelo usuário, com labels coloridas personalizáveis, visualização e download do PDF original, deduplicação automática por conteúdo SHA-256 e reutilização direta nas análises/entrevistas.
...
- **Histórico** — listagem e detalhamento das análises, otimizações e entrevistas já realizadas pelo usuário.
- **Conta** — edição de dados de perfil (nome, telefone, profissão), alteração de senha e exclusão de conta com todos os dados relacionados.
...
| RF08 | O sistema deve manter histórico de análises, otimizações e entrevistas por usuário, com listagem paginada e tela de detalhe. |
...
| RF11 | O sistema deve manter uma biblioteca centralizada de currículos do usuário, com deduplicação automática por conteúdo, labels coloridas personalizáveis e armazenamento do PDF original para visualização e download. |
| RF12 | O sistema deve permitir que o usuário edite seus dados de perfil (nome, telefone, profissão), altere sua senha e exclua permanentemente sua conta e todos os dados relacionados. |
...
| RNF06 | **Persistência**: dados de usuários, análises, otimizações, sessões de chat, entrevistas e currículos salvos devem ser persistidos em banco relacional (PostgreSQL), com schema versionado via migrations (Alembic). |
...
- **`src/services/`** — regras de negócio puras (prompts de IA, parsing de resposta da LLM, geração de PDF, abstração dos backends de LLM, gestão de currículos e perfil de conta). Pensadas para serem testáveis isoladamente da camada HTTP.
...
- **`src/app.py`** — *application factory* (`create_app()`): carrega config, inicializa extensões (SQLAlchemy, Flask-Mail, Flask-Migrate, Flask-Limiter), registra os 7 blueprints, define geração de `request_id` por requisição e handler de erro 429.
...
O repositório inclui `prompt-injection.txt` como caso de teste manual dessa defesa (ver seção [9. Testes realizados](#9-testes-realizados)).
...
| **ReportLab** | Geração de PDF | Biblioteca de baixo nível com controle fino sobre layout, necessário para suportar 3 templates visuais distintos de currículo (clássico, moderno, executivo), o relatório de entrevista e a conversão de DOCX/TXT para PDF na biblioteca de currículos. |
| **Pillow** | Processamento de imagens | Suporte a foto no cabeçalho do currículo otimizado (redimensionamento, conversão de formato antes de inserção no PDF via ReportLab). |
| **python-docx** | Leitura e conversão de DOCX | Extração de texto de arquivos `.docx` sem dependências externas pesadas; também usado pelo `conversor_pdf.py` para converter currículos DOCX para PDF. |
...
|---|---|---|
| `users` | `id` (UUID), `name`, `email` (único), `password` (hash), `reset_token`, `reset_token_expires_at`, `telefone`, `profissao`, `proximo_indice_cor` | Senha com `werkzeug.security.generate_password_hash`. Token de reset expira em 1h. `proximo_indice_cor` é um contador crescente para distribuição round-robin automática das cores de label dos currículos. |
| `curriculo` | `id`, `user_id` (FK), `label` (único por usuário), `cor`, `hash_conteudo` (SHA-256), `texto`, `arquivo_pdf` (LargeBinary), `arquivo_nome`, `arquivo_mimetype`, `criado_em` | Paleta de 30 cores fixas. Índice composto `(user_id, hash_conteudo)` para deduplicação eficiente. Constraint única `(user_id, label)`. PDF original armazenado como binário. |
| `analise` | `id`, `user_id` (FK), `curriculo_id` (FK opcional), `titulo`, `score_total`, `criterios` (JSON), `pontos_fortes/fracos` (JSON), `sugestoes` (JSON), `palavras_chave_faltando` (JSON), `certificados_sugeridos` (JSON), `texto_original`, `vaga` | Resultado de uma análise ATS. `titulo` gerado automaticamente pela LLM. |
...
| `entrevista` | `id`, `user_id` (FK), `curriculo_id` (FK opcional), `titulo`, `curriculo_arquivo`, `vaga_descricao`, `numero_perguntas`, `plano_entrevista` (JSON), `status` (`em_planejamento`/`em_andamento`/`concluida`), `relatorio_final` (JSON) | `titulo` gerado automaticamente pela LLM. |
...
- **Deduplicação de currículos por SHA-256**: currículos com o mesmo texto (independente de nome de arquivo ou data de envio) compartilham um único registro na tabela `curriculo`, referenciado por `analise.curriculo_id` e `entrevista.curriculo_id`.
- O schema é versionado em `migrations/versions/` (Alembic) com 4 migrations sequenciais: `0001_initial` (schema completo inicial), `0002_entrevista_titulo`, `0003_curriculo_arquivo_pdf`, `0004_user_perfil_extra`. `src/db_init.py` é um caminho alternativo (`db.create_all()` + `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`) usado em ambientes onde rodar `alembic upgrade head` é menos direto.
...
| Biblioteca de currículos | `curriculos.html` | Listagem de todos os currículos salvos do usuário com label, cor e data. Permite renomear label, alterar cor da paleta, visualizar o PDF original em um iframe, baixar o PDF e apagar o currículo. |
| Histórico | `historico.html`, `historico_detalhe.html` | Listagem paginada de análises/otimizações/entrevistas anteriores e tela de detalhe de cada uma. |
...
| Conta | `conta.html` | Edição de dados de perfil (nome, telefone, profissão), alteração de senha com verificação da senha atual, e exclusão permanente de conta com confirmação por senha. |
...
Browser envia arquivo + vaga
  ...
  → salva/recupera Curriculo por hash SHA-256
  → monta prompt
  → chama LLM
  → extrai JSON da resposta
  → persiste em Analise (com referência ao Curriculo)
...
### Fluxo principal: Upload e gestão de currículos

```
Usuário faz upload de arquivo em /analisar ou /entrevista
  → texto é extraído (pdftotext / python-docx / leitura direta)
  → SHA-256 do texto é calculado
  → se hash já existe para esse usuário → reutiliza Curriculo existente
  → se hash é novo → gera label via LLM, converte para PDF (se não for PDF), persiste Curriculo
  → referência curriculo_id é salva na Analise ou Entrevista
```
...
| `analisar.py` | `/` | `GET/POST /analisar` 🔒 ⏱, `GET /analises` 🔒, `PATCH /analises/<id>/titulo` 🔒, `DELETE /analises/<id>` 🔒, `GET /historico` 🔒, `GET /historico/<id>` 🔒, `GET /analises/<id>` 🔒, `POST /otimizar` 🔒 ⏱, `GET/POST /otimizar/pdf` 🔒 ⏱ |
| `chat.py` | `/` | `POST /chat` 🔒 ⏱ (streaming SSE), endpoints de gestão de sessões de conversa (criar, listar, fixar, renomear, excluir) |
| `entrevista.py` | `/entrevista` | `POST /gerar-plano` 🔒 ⏱, `GET /lista` 🔒, `GET /<id>` 🔒, `PATCH /<id>/titulo` 🔒, `DELETE /<id>` 🔒, `POST /<id>/responder` 🔒 ⏱, `POST /<id>/finalizar` 🔒, `GET /<id>/exportar-pdf` 🔒 |
| `curriculo.py` | `/curriculos` | `GET /` 🔒 (página), `GET /lista` 🔒, `GET /cores` 🔒, `GET/<id>` 🔒, `PATCH /<id>/label` 🔒, `PATCH /<id>/cor` 🔒, `DELETE /<id>` 🔒, `GET /pdf/<id>` 🔒, `GET /download/<id>` 🔒 |
| `conta.py` | `/` | `GET /conta` 🔒, `POST /conta/dados` 🔒 ⏱, `POST /conta/senha` 🔒 ⏱, `POST /conta/excluir` 🔒 ⏱ |
...
| `curriculo.py` | Geração automática de label via LLM, garantia de unicidade, deduplicação por hash SHA-256 e gerenciamento da paleta de cores. |
| `conversor_pdf.py` | Conversão de currículos `.docx` e `.txt` para PDF usando ReportLab para armazenamento na biblioteca. |
| `conta.py` | Atualização de dados de perfil, alteração de senha com verificação e exclusão de conta. |
...
- **PDFs dos currículos armazenados no banco**: a coluna `curriculo.arquivo_pdf` (`LargeBinary`) armazena os binários diretamente no Postgres. Isso simplifica o deploy (sem volume externo), mas pode aumentar significativamente o tamanho do banco em uso intensivo — trade-off aceitável para um MVP.
