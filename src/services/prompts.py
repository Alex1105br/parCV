...
def build_prompt_ats(curriculo, vaga=None):
    ...
    return (
        ...
        "Seja CONCISO em todos os campos de texto — frases curtas e diretas, sem repetir a mesma ideia em campos "
        "diferentes. Isso é importante porque a resposta tem um limite de tokens e precisa caber inteira.\n"
        "Analise o currículo com base nos critérios abaixo. Para CADA critério, dê uma nota, um motivo "
        "(1 frase curta, citando algo concreto do currículo — uma experiência, skill, ou a ausência dela — nunca uma frase genérica como 'currículo bem estruturado'), "
        ...
        "e explicar em poucas palavras o impacto daquele ponto na nota do critério — ex: 'Falta testes automatizados, exigidos pela vaga'):\n"
        ...
        "- 1 a 2 itens curtos em cada lista, podendo ser vazia se não houver nada relevante para aquele lado.\n"
        "- Cada item deve citar uma experiência, skill ou trecho REAL do currículo (ou a ausência dele) — nunca uma afirmação vaga.\n"
        "- Cada item deve terminar explicando em poucas palavras o impacto na avaliação daquele critério específico.\n"
        "\n"
        ...
        "- 'resumo': 2 frases diretas e curtas. Se a aderência for baixa ou média, diga EXPLICITAMENTE por que o candidato não é forte para esta vaga, citando a(s) lacuna(s) real(is) (ex: 'Você não tem experiência em desenvolvimento Android nem nas tecnologias AOSP e Qualcomm exigidas, então hoje você não é competitivo para esta vaga especificamente'). Não suavize a lacuna para parecer mais gentil.\n"
        "- 'vagas_recomendadas': 2-3 tipos de vaga/perfil onde este candidato teria MAIS chance real, com base nas skills e experiências que ele de fato tem (ex: 'Desenvolvedor Backend Python/C++' se o currículo mostra isso) — não repita a vaga analisada.\n"
        "- 'motivo_recomendacao': 1 frase curta conectando as vagas recomendadas às experiências/skills REAIS do currículo que as sustentam.\n"
        ...
        "\n"
        "Inclua em 'palavras_chave_faltando' (máximo 6 itens) as palavras-chave relevantes extraídas da vaga que estão ausentes no currículo. "
        ...
        "Inclua em 'certificados_sugeridos' até 3 certificações reconhecidas pelo mercado que agregariam valor ao candidato para esta vaga. "
